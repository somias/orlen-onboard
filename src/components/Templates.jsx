...
export function Templates({ onUse }) {
  return (
    <div className="space-y-4">
      <Eyebrow>Onboarding templates</Eyebrow>
        ...
        {Object.values(TEMPLATES).map((tpl) => {
          ...
          return (
            <div key={tpl.key} className="flex flex-col rounded-xl border border-slate-200 bg-white p-4">
              ...
              <div className="mt-1 text-xs text-slate-400">{trackCount} tracks · {itemCount} tasks</div>
              <div className="mt-3 flex flex-wrap gap-1.5">
                {Object.keys(tpl.tracks).map((t) => (
                  <span
                    key={t}
                    className={`rounded px-1.5 py-0.5 text-[11px] ${t === "Compliance & regulatory" ? "bg-amber-50 text-amber-700" : "bg-slate-50 text-slate-500"}`}
                  >
                    {t}
                  </span>
                ))}
              ...
              >
                Create hire <ChevronRight size={15} />
              </button>
            </div>
          );
        })}
